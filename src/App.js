...
function App() {
  ...
  const onAskQuestion = () =>{
    ...
    const filteredQuestions = sampleJson.find(question => question.question.toLowerCase().includes(search.toLowerCase()))
    const result = filteredQuestions.response
    setTimeout(() => {
      setChat((prev)=>{
        return [...prev, result]
      })
    ...
    <div className="App">
      ...
      <div className='chat'>
        <div className='chat-box'>
          {chat.length > 0 && chat.map((item, i)=> <div key={i} className='message'>
              <p>{item}</p>
            </div>)}
        </div>
